...
def check_domain_system(domain):
    ...
    is_registered = False # Default to false until proven true
    
    try:
        # TIMEOUT ADDED: We give whois exactly 3 seconds to reply
        proc = subprocess.run(
            ["whois", domain], 
            capture_output=True, 
            text=True, 
        ...
        # If we get here, whois worked. Let's parse it.
        is_registered = True # Assume registered unless we find "Not Found" text
        ...
        # --- FALLBACK: DNS CHECK ---
        # Whois timed out. Let's check if the domain has an IP address.
        try:
            socket.gethostbyname(domain)
            # If this line succeeds, the domain has an IP -> It is Registered.
            is_registered = True
            result["Registration"] = "REGISTERED (DNS Found)"
        except socket.gaierror:
            # DNS failed too. Likely unregistered, but we mark as timeout/unknown to be safe.
            is_registered = False
    ...
    # 2. Check HTTP (Web Status) - Only if we think it's registered
    if is_registered:
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (compatible; DomainChecker/1.0)'}
            url = f"http://{domain}"
            
            # Use HEAD first for speed (2 sec timeout)
...
st.write("Using system `whois` with DNS fallback and timeout protection.")
...
if st.button("Start Checking"):
    ...
    def color_registration(val):
        if 'REGISTERED' in val: # Matches "REGISTERED" and "REGISTERED (DNS Found)"
            return 'color: #28a745; font-weight: bold' # Green
        elif 'UNREGISTERED' in val:
            return 'color: #dc3545; font-weight: bold' # Red
        else:
            return 'color: #ffc107; font-weight: bold' # Orange (Timeout/Unknown)
    ...
    for i, domain in enumerate(clean_list):
        ...
        # Shorter delay (1s-2.5s) since we have timeouts now
